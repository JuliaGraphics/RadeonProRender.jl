using GeometryTypes, Colors, GLWindow, GLVisualize, StaticArrays
...
@rpr_wrapper_type MaterialSystem rpr_material_system ContextCreateMaterialSystem (context.x, typ) FireRenderObj
@rpr_wrapper_type MaterialNode rpr_material_node MaterialSystemCreateNode (matsystem.x, typ) FireRenderObj
@rpr_wrapper_type Camera rpr_camera ContextCreateCamera (context.x,) FireRenderObj
@rpr_wrapper_type Scene rpr_scene ContextCreateScene (context.x,) FireRenderObj

@rpr_wrapper_type Context rpr_context CreateContext (api_version, pluginIDs, pluginCount, creation_flags, props, cache_path) FireRenderObj

"""
Constructor which only uses creation flags and defaults to opencl.
"""
function Context(flags)
    Context(API_VERSION, C_NULL, 0, flags, C_NULL, C_NULL)
end
"""
Empty constructor, defaults to using opencl and the GPU0
"""
function Context()
    Context(CREATION_FLAGS_ENABLE_GPU0)
end
...
@rpr_wrapper_type EnvironmentLight rpr_light ContextCreateEnvironmentLight (context.x,) AbstractLight
@rpr_wrapper_type PointLight rpr_light ContextCreatePointLight (context.x,) AbstractLight
@rpr_wrapper_type DirectionalLight rpr_light ContextCreateDirectionalLight (context.x,) AbstractLight
@rpr_wrapper_type SkyLight rpr_light ContextCreateSkyLight (context.x,) AbstractLight
@rpr_wrapper_type SpotLight rpr_light ContextCreateSpotLight (context.x,) AbstractLight
...
function Shape(context::Context, mesh::AbstractGeometry, args...)
    ...
    m = ContextCreateMesh(
        ...
        iraw, sizeof(rpr_int),
        iraw, sizeof(rpr_int),
        fill(rpr_int(3), length(i)), length(i)
    )
    Shape(m)
end
"""
...
function rpr_image_format{T<:Union{Colorant, AbstractFloat}}(::Type{T})
    rpr_image_format(
...
"""
function rpr_image_desc{T,N}(image::Array{T, N})
...
function Image{T, N}(context::Context, image::Array{T, N})
    ...
    context.x, rpr_image_format(image),
...
function setradiantpower!(
    ...
    PointLightSetRadiantPower3f(light.x, rpr_float(r), rpr_float(g), rpr_float(b))
end
function setradiantpower!(
        light::SpotLight,
        r::AbstractFloat, g::AbstractFloat, b::AbstractFloat
    )
    SpotLightSetRadiantPower3f(light.x, rpr_float(r), rpr_float(g), rpr_float(b))
end
function setradiantpower!(
        light::DirectionalLight,
        r::AbstractFloat, g::AbstractFloat, b::AbstractFloat
    )
    DirectionalLightSetRadiantPower3f(light.x, rpr_float(r), rpr_float(g), rpr_float(b))
end
...
function setintensityscale!(light::EnvironmentLight, intensity_scale::AbstractFloat)
    EnvironmentLightSetIntensityScale(light.x, rpr_float(intensity_scale))
end
...
set!(context::Context, parameter::AbstractString, f::AbstractFloat) =
    ContextSetParameter1f(context.x, ascii(parameter), rpr_float(f))
set!(context::Context, parameter::AbstractString, ui::Unsigned) =
    ContextSetParameter1u(context.x, ascii(parameter), rpr_uint(ui))
set!(context::Context, aov::Unsigned, fb::FrameBuffer) =
    ContextSetAOV(context.x, rpr_aov(aov), fb.x)
...
function set!{T<:StaticVector{4}}(base::MaterialNode, parameter::AbstractString, f::T)
...
function setdisplacementscale!(shape::Shape, scale::AbstractFloat)
    ShapeSetDisplacementScale(shape.x, rpr_float(scale))
end
"""
Sets the subdivions for a shape
"""
function setsubdivisions!(shape::Shape, subdivions::Integer)
    ShapeSetSubdivisionFactor(shape.x, rpr_uint(subdivions))
end
...
function get(scene::Scene, ::Type{EnvironmentLight})
    EnvironmentLight(SceneGetEnvironmentLight(scene::rpr_scene))
end
"""
